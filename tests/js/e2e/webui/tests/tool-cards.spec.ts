...
test.describe("Tool Card Visualization", () => {
  ...
  test("should render reasoning card with Thought label and toggle", async ({
    page,
  }) => {
    ...
    // Header and toggle visible
    await expect(reasoningCard.locator(".reasoning-header")).toBeVisible();
    await expect(reasoningCard.locator(".reasoning-toggle")).toBeVisible();
...
test.describe("Tool Result Formatting", () => {
  ...
  test("should format grep result with match count", async ({ page }) => {
    const result = {
      pattern: "test",
      match_count: 1,
      matches: "app.py:10: test function",
    };
  ...
  test("should format todo result as table", async ({ page }) => {
    const result = {
      todos: [
        ...
        { content: "Write tests", status: "done", priority: "medium" },
      ],
    };
  ...
  test("should format lsp result with diagnostic summary", async ({ page }) => {
    const result = {
      diagnostics: [{ severity: 2, message: "Unused import" }],
      formatted_output: "app.py:5: Unused import",
    };
  ...
  test("should format web search result with answer", async ({ page }) => {
    const result = {
      answer: "The sky is blue",
      sources: [{ title: "Sky color", url: "https://example.com/sky" }],
    };
...
test.describe("LLM Error Display", () => {
  test("should render LLM error card with type, details, and provider metadata", async ({
    page,
  }) => {
    ...
    await expect(errorHeader).toContainText("AuthenticationError");
    ...
    await expect(errorDetails).toContainText("Invalid API key");

    // Metadata section with provider and model
