...
async def test_pyright_real_server_initialization(tmp_path: Path):
    ...
    try:
        ...
        assert handle["initialization"] is not None
...
""")
    ...
    try:
        ...
        assert len(diagnostics) > 0, (
            "Expected to receive diagnostics from pyright server"
        )
        ...
        error_diagnostics = [
            d for d in diagnostics if d.get("severity", 0) in [1, 2]
        ]  # 1=error, 2=warning
        assert len(error_diagnostics) > 0, (
            "Expected to receive at least one error or warning diagnostic"
        )
...
""")
    ...
    try:
        ...
        [d for d in diagnostics if d.get("severity", 0) in [1, 2]]  # 1=error, 2=warning
...
async def test_pyright_get_command_returns_local_binary():
    ...
    if exec_path and exec_path.exists():
        ...
        assert Path(command[1]).exists(), (
            f"pyright-langserver.js should exist at {command[1]}"
        )
    else:
