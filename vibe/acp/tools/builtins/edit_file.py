...
class AcpEditFileState(BaseToolState, AcpToolState):
    file_backup_content: str | None = None
    file_backup_encoding: str = "utf-8"


class EditFile(CoreEditFileTool, BaseAcpTool[AcpEditFileState]):
    state: AcpEditFileState
    prompt_path = VIBE_ROOT / "core" / "tools" / "builtins" / "prompts" / "edit_file.md"

    @classmethod
    def _get_tool_state_class(cls) -> type[AcpEditFileState]:
    ...
    def tool_call_session_update(cls, event: ToolCallEvent) -> SessionUpdate | None:
        if not isinstance(event.args, EditFileArgs):
            return fallback_tool_call(event, "edit_file")
    ...
    def tool_result_session_update(cls, event: ToolResultEvent) -> SessionUpdate | None:
        if failure := failed_tool_result(event, EditFileResult):
            return failure
