...
class LSPClient:
    ...
    def _check_server_alive(self) -> bool:
        ...
        if self.process.returncode is not None:
            ...
            logger.debug(
                f"LSP server process exited with code: {self.process.returncode}"
            )
            return False
    ...
    async def send_request(
        self, method: str, params: LSPRequestParams | None = None
    ) -> LSPResponse:
        self.message_id += 1
        request_id = self.message_id

        request: LSPMessage = {"jsonrpc": "2.0", "id": request_id, "method": method}
    ...
    async def send_notification(
        self, method: str, params: LSPNotificationParams | None = None
    ) -> None:
        notification: LSPMessage = {"jsonrpc": "2.0", "method": method}
        if params is not None:
    ...
    async def initialize(
        ...
        params: LSPRequestParams = {"capabilities": capabilities}
    ...
    async def text_document_did_open(
        self, uri: str, text: str, language_id: str
    ) -> None:
    ...
    async def text_document_did_change(
        self, uri: str, text: str, version: int = 2
    ) -> None:
    ...
    async def _wait_for_publish_diagnostics(
        self, uri: str, timeout: float = 0.3
    ) -> list[dict[str, Any]]:
        ...
        if (
            uri in self.diagnostics_refreshed
            and self.diagnostics_refreshed[uri] is None
        ):
            ...
            while asyncio.get_event_loop().time() - start_time < timeout:
                ...
                if (
                    uri in self.diagnostics_refreshed
                    and self.diagnostics_refreshed[uri] is not None
                ):
                    # Diagnostics were refreshed, return them
                    logger.debug(
                        f"publishDiagnostics was refreshed: {self.diagnostics.get(uri, [])!s}"
                    )
    ...
    async def _handle_response(self, response: LSPMessage) -> None:
        ...
        if "id" not in response:
            ...
            logger.debug(
                f"LSP Notification received: {response.get('method', 'unknown')}"
            )
            ...
            if response.get("method") == "textDocument/publishDiagnostics":
                ...
                if uri and diagnostics is not None:
                    ...
                    logger.debug(
                        f"Received diagnostics for {uri}: {len(diagnostics)} issues"
                    )
            ...
            # Log unknown notifications for debugging
                ...
                logger.debug(
                    f"LSP Unknown notification: {response.get('method', 'unknown')}"
                )
    ...
    async def _extract_message_from_buffer(
        self, buffer: bytes
