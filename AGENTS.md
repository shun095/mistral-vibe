...
EOF
```
- Always commit with `git commit -F - <<'EOF'`. The `-F -` flag is required — without it git ignores stdin. Never use `git commit -m` with multi-line messages — single-quote errors silently drop `Co-Authored-By`.
